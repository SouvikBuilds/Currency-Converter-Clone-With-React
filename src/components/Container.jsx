...
const Container = () => {
    ...
    async function convertCurrency() {
        if (!fromInput || !toInput || !amount) {
            alert("Please Fill The Details First")
            return
        }

        const apiKey = "e3b5823bccc571ccf87088b0"
        const apiUrl = `https://v6.exchangerate-api.com/v6/${apiKey}/latest/${fromInput}`
    ...
    useEffect(() => {
        convertCurrency()
    }, [fromInput, amount, toInput, isSwapped])

    return (
        <div
            className="min-h-screen w-full bg-cover bg-center bg-no-repeat bg-fixed flex items-center justify-center p-4"
            style={{
                backgroundImage: "url(https://media.istockphoto.com/id/539236471/photo/wheel-of-currency.jpg?s=612x612&w=0&k=20&c=0k-P5xuqCbamAkWG1f2h9KdvkiYwHRfEnyY-glGOEwU=)"
            }}
        >
            <div className='container bg-black rounded-lg shadow-lg text-center flex flex-col justify-center items-center p-6 w-full max-w-[600px] mx-auto'>
                ...
                <div className='flex flex-col gap-2 items-start justify-center mt-[50px] w-full'>
                    <label htmlFor="fromInput" className='text-xl text-white font-semibold'>
                        From
                    </label>
                    <div className='flex flex-col sm:flex-row gap-2 items-center w-full'>
                        <input
                            ...
                            name=""
                            id="fromInput"
                            className='px-4 py-2 rounded-lg shadow-lg w-full sm:w-[300px]'
                            readOnly
                        />
                        <select
                ...
                <div className="amount flex flex-col justify-center items-start gap-2 mt-[20px] w-full">
                    <label htmlFor="inputAmount" className='text-xl text-white font-semibold'>Amount</label>
                    <input
                        type="number"
                        value={amount}
                        ...
                        className='px-5 py-2 rounded-lg shadow-lg w-full'
                    />
                </div>
                ...
                <div className="result mt-[20px] px-4">
                    {currencyData && (
                        <h2 className='text-lg sm:text-xl text-center font-semibold text-white break-words'>
                            {amount} {fromInput} = {amount * currencyData.conversion_rates[toInput]} {toInput}
                        </h2>
                    )}
                </div>
            </div>
